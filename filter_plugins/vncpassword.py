...
def vnc_encrypt(passwd):
    key = bytearray([23,82,107,6,35,78,88,7])
    plain = bytearray((passwd + '\x00'*8)[:8], 'utf8')
    ek = deskey(key, False)
    dk = deskey(key, True)
    hexdata = desfunc(plain, ek)
    return ",".join("{:02x}".format(ord(c)) for c in hexdata)
